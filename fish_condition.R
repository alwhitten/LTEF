...
#---Notes---#
# LTEF annual report usually only calculates Wr for Silver Carp in the lower Illinois River by combining LTEF sites and LTRM La Grange efishing sites. 
...
#---Wr Summary Stats---#
WrSum <- fish %>% group_by(YEAR) %>% summarize(LOCATION_CODE=n(), mean_Wr=mean(Wr), sd=sd(Wr), se=sd/sqrt(LOCATION_CODE), RSE=se/mean*100) %>% as.data.frame()
############################
